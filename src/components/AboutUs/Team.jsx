...
const TeamMemberCard = ({
  item,
  behavior, // "scroll" | "toggle"
  t,
  scrollToId,
  className = "",
  collapsedPanelClasses = "",
  expandedPanelClasses = "",
  readMoreWrapperClasses = "",
}) => {
  const [isExpanded, setIsExpanded] = React.useState(false);

  const handleMouseLeave = () => {
    if (behavior === "toggle") setIsExpanded(false);
  };
    ...
    >
      ...
      <div
        ...
        className={`
          ...
          tablet:h-[60vw]
          ${
            behavior === "toggle" && isExpanded
              ? `translate-y-[0%] h-[100%] overflow-y-scroll ${expandedPanelClasses}`
              : `translate-y-[70%] group-hover:translate-y-[0%] overflow-hidden ${collapsedPanelClasses}`
          }
        `}
      >
        ...
        {behavior === "toggle" && isExpanded && Array.isArray(item.para) && (
          <div className="w-full py-[3vw] px-[1vw] space-y-[1vw] tablet:px-[1.5vw]">
            {item.para.map((paraItem, index) => (
              <p
                key={index}
                className="text-[1.2vw] mobile:text-[3.5vw] tablet:text-[2vw] mobile:leading-[1.6]"
              >
                {paraItem}
              </p>
            ))}
        ...
        <div
          className={`w-fit mt-[1vw] pb-[2vw] cursor-pointer mobile:pb-[5vw] ${
            readMoreWrapperClasses ||
            (behavior === "scroll"
              ? "pl-[28vw] mobile:pl-[50vw]"
              : "pl-[18vw] tablet:pl-[30vw] mobile:pl-[50vw]")
          }`}
    ...
    <>
      <section className="w-screen h-full bg-[#ffffff]  pb-[5vw] mobile:overflow-hidden mobile:space-y-[8vw] tablet:pb-[7%] dark">
        ...
        mobile:rounded-[3vw] tablet:h-[60vw] mobile:mt-[-5vw] w-[28vw]`}
                  collapsedPanelClasses="h-[35vw] mobile:h-[100vw] tablet:translate-y-[75%]"
                  readMoreWrapperClasses="pl-[18vw] tablet:pl-[30vw] mobile:pl-[50vw]"
                />
              ))}
