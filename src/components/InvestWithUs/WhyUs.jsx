...
const WhyUS = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      setIsMobile(window.innerWidth <= 541); // adjust based on your `mobile` breakpoint
    };
  ...
  return (
    <section className="bg-[#FFFFFF] py-[7vw] mobile:py-[10vw] dark w-screen overflow-hidden">
      <div className="px-[5%] mobile:px-[5%]">
        <h2
          data-title-anim
          className="heading-1 mb-[6vw] mobile:leading-[1.2] tablet:!text-[5vw] mobile:!text-[10.8vw] mobile:mb-[8vw]"
        >
          {t("whyUsHead")}
        </h2>
        <div className="w-full mobile:overflow-x-scroll">
          <div className="flex gap-[1.3vw] mobile:gap-y-[8vw] mobile:w-fit mobile:gap-[4vw] ">
            {text.map((item, index) => (
              <div
                // Main Card
                key={index}
                className="w-[17vw] h-[22vw] relative group duration-500 transition-transform mobile:w-[43vw] mobile:h-[60vw]"
                style={{
              ...
              >
                ...
                >
                  <Image
                    ...
                    alt="Image"
                  />
                  <div className="absolute bg-[#F2F2E9] top-0 left-0 bottom-0 right-0 opacity-0 group-hover:opacity-100 z-[5] transition-all duration-500 ease py-[1vw] flex flex-col justify-between h-full pr-[1.5vw] px-[1vw] w-full mobile:hidden">
                ...
                <div
                  className={`bg-[#747977] absolute top-0 left-[calc(17vw+1.3vw)] w-[calc(17vw*4+3.9vw)] h-full duration-500 mobile:hidden ${
        ...
        <div className="hidden mobile:block w-full h-[70vw] bg-[#747977] mt-[8vw]">
          <div className="w-full h-full relative flex flex-col justify-start gap-[1vw] p-[1.5vw] text-white">
            {text.map((item, index) => (
              <div className={`${index==currentMobileIndex?"":"hidden"} flex flex-col gap-[4vw] px-[2vw] py-[4vw]`}>
                ...
                <p className="text-[6.2vw] !leading-[1.2] !font-display">
                  {item.title}
                </p>
                <p className="text-[4.1vw]">{item.para}</p>
              </div>
            ))}
          </div>
        </div>
